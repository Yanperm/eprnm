...
class BookingModel extends CI_Model
{
    ...
    public function find_with_page($param)
    {
        ...
        $condition = "1=1 and CLINICID = ".$this->session->userdata('id');
        ...
        $count_condition = $this->db->from('tbbooking')->where($condition)->count_all_results();
        $count = $this->db->from('tbbooking')->count_all_results();
    ...
    public function getDataListNew($clinicId, $rowperpage, $rowno)
    {
        ...
        $query = $this->db->query(
            ...
            SELECT * FROM tbbooking as booking
            INNER join tbmembers as member on member.MEMBERIDCARD = booking.MEMBERIDCARD OR member.IDCARD = booking.IDCARD
            where (TYPE = 0 AND booking.CLINICID = "' . $clinicId . '" AND booking.create_date > "' . $days_ago . '" AND BOOKTIME != "")
            OR (STATUS !=2 AND TYPE != "0" AND booking.CLINICID = "' . $clinicId . '" AND booking.create_date > "' . $days_ago . '" )
            order by booking.create_date desc
            limit ' . $rowno . ',' . $rowperpage
        );
