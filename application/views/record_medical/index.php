<div class="user-profile" id="vue-root"> 
                                    ...
                                    <div class="col-12 col-lg-12">
                                        <div class="card">
                                            <vs-row>
                                                ...
                                                <a href="<?php echo base_url('recordMedical/sticker')?>?id=<?php echo $bookingId;?>" target="_blank"><vs-button color="success" type="border" icon="print"> พิมพ์ฉลากยา</vs-button></a>    
                                                <vs-button @click="popupActive=true,action='insert'" color="primary" type="filled" icon="add">เพิ่มข้อมูล</vs-button>
                                                </vs-col>
                                            </vs-row>
    ...
    <vs-popup fullscreen class="holamundo" title="Medical Record" :active.sync="popupActive">
        <p>กรุณาเลือกยา</p>
        <v-row>
            <vs-table :sst="true" @search="handleSearchItem" @sort="handleSortItem" v-model="selectedItem" :total="totalItemsPro" :max-items="perPageItem" search :data="recordDataItem">
                <template slot="thead">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <vs-select autocomplete class="selectExample" label="ครั้งล่ะ *" v-model="countUnit">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in optionsCountUnit" />
                            </vs-select>
                        </div>
                        <div class="col-md-4">
                            <vs-select autocomplet  class="selectExample" label="หน่วย *" v-model="callingUnit">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in optionsCallingUnit" />
                            </vs-select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <vs-select autocomplete class="selectExample" label="ความถี่ *" v-model="fregquency">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in optionsFregquency" />
                            </vs-select>
                        </div>
                        <div class="col-md-6">
                            <vs-select autocomplet  class="selectExample" label="เวลา *" v-model="meal">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in optionsMeal" />
                            </vs-select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <vs-select autocomplete class="selectExample" label="ข้อแนะนำ *" v-model="suggestion">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in optionsSuggestion" />
                            </vs-select>
                        </div>
                    </div>
                </div>
                ...
                axios.get("recordMedical/getOptiondata").then((response) => {
                    if (response.data.result) {
                        for (let i = 0; i < response.data.data.countUnit.length; i++) {
                            this.optionsCountUnit.push({text:response.data.data.countUnit[i].detail,value : response.data.data.countUnit[i].detail},);
                        }
                        this.countUnit = this.optionsCountUnit[0].value;

                        for (let i = 0; i < response.data.data.callingUnit.length; i++) {
                            this.optionsCallingUnit.push({text:response.data.data.callingUnit[i].detail,value : response.data.data.callingUnit[i].detail},);
                        }
                        this.callingUnit = this.optionsCallingUnit[0].value;


                        for (let i = 0; i < response.data.data.fregquency.length; i++) {
                            this.optionsFregquency.push({text:response.data.data.fregquency[i].detail,value : response.data.data.fregquency[i].detail},);
                        }
                        this.fregquency = this.optionsFregquency[0].value;

                        for (let i = 0; i < response.data.data.meal.length; i++) {
                            this.optionsMeal.push({text:response.data.data.meal[i].detail,value : response.data.data.meal[i].detail},);
                        }
                        this.meal = this.optionsMeal[0].value;

                        for (let i = 0; i < response.data.data.suggestion.length; i++) {
                            this.optionsSuggestion.push({text:response.data.data.suggestion[i].detail,value : response.data.data.suggestion[i].detail},);
                        }
            ...
            getDataItem() {
                axios.get("recordMedical/getProduct", {
                    ...
                    if (response.data.result) {
                        ...
                        this.paginationItem.last_page = Math.ceil(parseInt(response.data.total) / this.perPageItem);
                    } else {
