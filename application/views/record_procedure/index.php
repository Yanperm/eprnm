<div class="user-profile" id="vue-root">
                                ...
                                <div class="tab-pane active" role="tabpanel">
                                    <div class="mt-2 mb-2">
                                        <div class="mt-2 mb-2">
                                            ...
                                            <a href="<?php echo base_url('recordProcedure');?>?id=<?php echo $member->MEMBERIDCARD;?>&booking_id=<?php echo $bookingId;?>" class="btn btn-space btn-secondary btn-big active"><i class="icon mdi mdi-airline-seat-flat-angled"></i> Procedure</a>
                                            ...
                                            <vs-table :sst="true" @search="handleSearch" @sort="handleSort" v-model="selected" :total="totalItems" :max-items="perPage" search :data="recordData">
                                                ...
                                                <template slot-scope="{data}">
                                                    ...
                                                    <vs-td :data="data[indextr].PH1">
                                                        {{data[indextr].PH1}} <p>{{data[indextr].PH2}}</p>
                                                    </vs-td>
    ...
    <vs-popup class="holamundo" title="รายการหัตถการในคลินิก" :active.sync="popupActive">
        <v-row>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            getData() {
                axios.get("recordProcedure/getData", {
                    params: {
                        search: this.search,
                        sortBy: this.sortBy,
            ...
            getDataItem() {
                axios.get("recordProcedure/getProcedure", {
                    params: {
            ...
            acceptAlert() {
                axios.post("recordProcedure/delete", {
                    id: this.id,
                }).then((response) => {
                    if (response.data.result) {
                        this.$vs.notify({
